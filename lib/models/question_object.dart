...
class QuestionObject {
  ...
  factory QuestionObject.fromMap(Map<String, dynamic> map) {
    ...
    for (var i = 0; i < map['contents'].length; i++) {
      String content = map['contents'][i];
      if (content.split(' ').last.contains('.png') || content.split(' ').last.contains('.jpg')) {
        contents.add(content.replaceAll(content.split(' ').last, ''));
        contents.add('img:${content.split(' ').last}');
      }
    }
    return QuestionObject(
      questionCount: int.parse((map['questionCount'] as String).replaceAll('Question ', '').replaceAll('.', '')),
      contents: contents,
